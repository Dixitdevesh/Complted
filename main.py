...
def listen():
     try: 
        with sr.Microphone() as source:
                print("Listening....")
                audio=r.listen(source,timeout=9,phrase_time_limit=6)
                print("Recognizing...")
...
def register_user():
    try:
        with open("users.csv", "a", newline='') as f:
            writer = csv.writer(f)
            speak("Please say a username.")
            username = listen()
            speak("Please say a password.")
            password = listen()
            ...
            try:
                ...
                while True:
                    if n==0:
                        ...
                        speak("Taking Command Please Speak ")
                        command=listen()
                        if command=="exit" or  "stop" in command:
                                print("Exiting.....")
                                speak("Exiting......")
                                break
                        else:
